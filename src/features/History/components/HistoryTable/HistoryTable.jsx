...
function HistoryTable({ historyList, onRemoveClick, onToolbarRemoveClick }) {
  ...
  const rows = historyList.map((history) =>
    createData(history.userName, history.formName, history.actionName, history.time, history.id)
  );
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <HistoryTableToolbar numSelected={selected.length} onAccept={handleToolbarAcceptRemove} />
        <TableContainer>
